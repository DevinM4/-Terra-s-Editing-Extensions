...
_lnbKeyCodes = _display displayCtrl 100;
_lnbKeyCodesHeader = _display displayCtrl 101;
_btnClear = _display displayCtrl 200;
...
switch _mode do {
	...
	case "displaykey":{
		...
		_display setVariable ["_lastKeys",_lastKeys +[_key]];
		...
		_hexValue = [str _key, 10, 16] call TER_fnc_convertNumber;
		_findInd = _dikList find _hexValue;
		_dikdef = if (_findInd > 0 && _findInd+1 < count _dikList) then {
			_dikList select (_findInd -1);
		} else {
			"?"
		};

		_ind = _lnbKeyCodes lnbAddRow [str _key, _hexValue, _dikdef];
		_lnbKeyCodes lnbSetCurSelRow _ind;
